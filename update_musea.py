...
def send_whatsapp(message: str) -> None:
    ...
    try:
        ...
        print(f"  WhatsApp verzonden (HTTP {resp.status_code}): {resp.text[:100]}")
    except Exception as e:
        print(f"  WhatsApp verzenden mislukt: {e}")
...
def main():
    ...
    print("-" * 40)
    status = "OK" if not write_errors else f"FOUTEN ({write_errors})"
    lines = [
        f"MuseumPass update {datetime.now().strftime('%d/%m/%Y')} - {status}",
        f"Gescraped: {len(museums)} | Bijgewerkt: {updated} | Nieuw: {new}",
    ]
    if new_museums_list:
    ...
    if scrape_errors:
        lines.append(f"Scrape-fouten: {scrape_errors}")
    send_whatsapp("\n".join(lines))
